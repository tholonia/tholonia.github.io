---
sitemap: false
title: "Ch. 14 - Applications"
image: /the_book/assets/chapters/images/014-APPLICATIONS.jpg
---
<div style='page-break-after: always; break-after: always;'></div>

# 14: APPLICATIONS

###### Tholonic patterns applied: from mathematical laws to the shape of all knowledge

##### **Synopsis:** Having identified tholonic patterns in physics and mathematics, this chapter applies those patterns outward to reveal what they mean in practice.  The tetrahedron is shown to unify radically different value systems — integers, imaginary numbers, *e*, and *π* — through its structural roles.  Newton's 2nd law is mapped directly onto the tholon, then extended via Ohm's law to describe social systems: ethics, laws, and societal strength are shown to follow the same archetypal formula as voltage, resistance, and current.  The mathematical meaning of operations (multiplication, division, squares) is reexamined in the tholonic context.  The chapter concludes by showing that every piece of knowledge forms a tetrahedron, and DNA's tetrahedral structure hints at deep tholonic roots underlying biology and information itself.

##### **Keywords:** tetrahedron, Newton's 2nd law, Ohm's law, society, ethics, laws, sustainability, imaginary numbers, meaning of math, DNA, tetrahedral DNA, shape of knowledge, N-D-C

## The Bigger Picture

Chapter 13 demonstrated that the same N-D-C architecture appears at every scale of physical reality, from quark charge distributions to molecular geometry to the growth constant *e*.  The question this chapter addresses is what those patterns mean in practice, when applied outward from physics and mathematics to social systems, cognition, and biology.
...
For example, if we only use values of ***i*** for pattern **g1**, using the values of ***i<sup>1</sup>***, ***i<sup>2</sup>***, ***i<sup>3</sup>***, we get values of *i*, *-1*, *-i*, which perfectly comply with pattern **g1** as i/(-1) = -i, i/(-i) = -1, and i × -1 = -i.  But this is a tetrahedron, so we need that 4<sup>th</sup> *white dot* point that needs to be a value that is fundamental to, and compatible with, all the other values.  That would be *i<sup>0</sup>*, which is 1 and the value that all these values are derived from.  And with that, all the formulas work.

If we continued with *i<sup>5</sup>*, *i<sup>6</sup>*, and so on, we would get the same values because no matter how many multiples of i are used, it can only result in *1*, *i*, *-1*, or *-i*, which is yet more validation of the tetrahedron model.  This pattern also tells us that the cardinal values of the points on a trigram are the exponential values of 1 (at the top), then 2 and 3, with the 4<sup>th</sup> point as 0 *after* the tholonic trigram has created its 3 children and folded into a tetrahedron.  So, while the instances of the pattern are different, the pattern shows how different types of functions are grouped by the base and side trigrams.
...
(For highly speculative thoughts on the meaning of squares and square roots, see *Meaning of Squares and Square Roots* in [Appendix C2, "Tholonic Math"](#Appendix-C2:-Tholonic-Math))
...
- 3 = ***society*** (strength of interaction) and equates to **current** (I) or **acceleration** (a)
...
- Ethics = 6, Laws = 2
- Society = ethics/laws = 6/2 = 3
- Work = ethics × society = 6 × 3 = 18

**Case 2 (Doubled ethics, same laws):**
- Ethics = 12 (doubled), Laws = 2 (unchanged)
- Society = ethics/laws = 12/2 = 6 (society doubles)
- Work = ethics × society = 12 × 6 = 72 (work quadruples!)

**Case 3 (Same ethics, doubled laws):**
- Ethics = 6 (unchanged), Laws = 4 (doubled)
- Society = ethics/laws = 6/4 = 1.5 (society reduced by half)
- Work = ethics × society = 6 × 1.5 = 9 (work cut in half)

**Ranking by work efficiency:**
...
1. **Most efficient: Case 2** - Work efficiency = 72/(12+2) = 72/14 = **5.14**  
   Doubled ethics with the same laws produces the highest efficiency, with work quadrupling while total input less than doubles.

2. **Moderately efficient: Case 1** - Work efficiency = 18/(6+2) = 18/8 = **2.25**  
   The baseline scenario shows reasonable efficiency with balanced ethics and laws.

3. **Least efficient: Case 3** - Work efficiency = 9/(6+4) = 9/10 = **0.9**  
   Doubling laws while keeping ethics constant cuts work in half and results in the poorest efficiency, producing less than one unit of work per unit of input.
...
Applied to these 3 cases, we get the following sustainability calculations:

**Case 1 (Baseline):**
- Formula: 6/2 or 18/6 or √(18/2)
- Calculation: (6 × 18 × √18)/(2 × 6 × √2) = 458.2/16.9 
- **Sustainability = 27.1**

**Case 2 (Doubled ethics):**
- Formula: 12/2 or 72/12 or √(72/2)
- Calculation: (12 × 72 × √72)/(2 × 12 × √2) = 7331.3/33.9
- **Sustainability = 216.3** (8× more sustainable than baseline!)

**Case 3 (Doubled laws):**
- Formula: 6/4 or 9/6 or √(9/4)
- Calculation: (6 × 9 × √9)/(4 × 6 × √4) = 162/48
- **Sustainability = 3.4** (8× less sustainable than baseline)
...
In any case, the twelve formulas of Newton's 2<sup>nd</sup> law show some potential when applied to other contexts and may offer a new perspective on the dynamics of culture and society that, if nothing else, raises some worthwhile questions.  As it applies here, it shows how the tholon can describe everything from atoms to culture as long as we know what data to apply.  Because the math involved in a tholon can be more than the reader might be interested in, more of this subject is covered in *Appendix C1, "Tholonic Math".*
...
**Negotiation = Contribution × Definition**
...
Applying different values and concepts can result in new data.  We experiment with this a little, but given it is outside the central theme of this book, we have put that section under "New data?" in *Appendix C1, "Tholonic Math"*
...
DNA's relationship with tetrahedra is particularly unique, given the creation of tetrahedrally structured DNA (a.k.a.  TDN).  TDN is the cutting edge of DNA nanotechnology and has various applications such as drug delivery, inhibiting certain gene expressions, and interfacing with biosensors.  These TDNs assemble themselves from scratch and offer greater strength and stability than traditional dual-strand DNA.  Given that there is little accessible information on TDNs for the layperson, there is not much more that can be said here other than they represent a significant advancement and will most likely play a role in future biotechnology, including potential applications in TDN-interfacing AI and DNA-integrated quantum computers.

<center><img src='../Images/tdn.png' style='width:100%'/></center>

Our simple trigrams of relationships, such as Newton's 2^nd^ law, form conceptual *tetrahedrons* or *tholons* of relationships.  Whereas before, it was stated that every piece of knowledge could be deconstructed into a trigram, by the same token, every piece of knowledge forms a tetrahedron.  From social ethics to DNA nanotechnology, the same structural template holds: three functional roles, one emergent fourth point, and a pattern that scales without distortion from the subatomic to the civilisational.

What this chapter has established is that the N-D-C pattern is not a feature of carefully chosen examples but a universal constraint.  Any system that must balance a generative source, a limiting definition, and an integrative contribution in order to sustain itself will exhibit this pattern, regardless of whether that system is a circuit, a society, a molecule, or a mind.  The details differ; the architecture does not.

Chapter 15 takes the most intimate version of that architecture, its expression in human consciousness, and asks what it means that the organ through which we perceive and reason is itself a tholon following the same rules as everything else it perceives and reasons about.
