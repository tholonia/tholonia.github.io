...
series: 1
chapter_title: INSTANCES
chapter_file: 013-INSTANCES.md
version: 8.0.89
status: 'Updated: Mar 26, 2026 v8.0.89'
subject: philosphy, geometry, mathematics, physics, consciousness, energy, chaos, order, reason, creation, evolution, archetypes, instances, dimensional progression, N-D-C pattern, quarks, proton, neutron, Euler's identity, Bernoulli, e, fractal self-similarity, tholonic structure
image: /the_book/assets/chapters/images/013-INSTANCES.jpg
---

<div style='page-break-after: always; break-after: always;'></div>
# 13: INSTANCES
###### Examples of how tholonic archetypes instantiate
##### **Synopsis:** An instance of an archetype can only exist within the constraints of its context.  This chapter examines what archetypes are, how they instantiate across dimensional frameworks, and how geometric principles underlie their manifestation.  The progression from 0-dimensional potential through multi-dimensional form is traced, along with the fractal self-similarity of tholonic structures across scales.  Concrete tholonic patterns are discovered in subatomic physics: the charge distribution of quarks maps precisely onto the N-D-C trigram, and molecular geometry mirrors tholonic folding.  The chapter concludes with Euler's identity and Bernoulli's formula for *e*, both of which reveal the tholonic pattern of creation, growth, and the singular uniqueness of the first tholon.

##### **Keywords:** archetypes, instances, dimensional progression, N-D-C pattern, quarks, proton, neutron, Euler's identity, Bernoulli, e, fractal self-similarity, tholonic structure

Chapter 12 established the geometric architecture of the thologram and closed by asking what reality is actually filled with.  The answer is instances: the concrete, observable expressions that emerge when archetypes encounter conditions that allow them to manifest.  That is the subject of this chapter.
...
- Each expression in a dimensional space will utilize all available dimensions.  If given the nature and scope of an instance that it will move *N* units, then in a 1-dimensional space, it will move *N* units in one dimension.  In a 2-dimensional space, it will move *N* units in 2 dimensions, and so on.  In our example, our line is moved 5 units in a 1-dimensional space.  In a 2D space, it moves *x* by 5 units and *y* by 5°.  In a 3D space, it would move *x* by 5 units, *y* by 5°, and *z* by 5°, and so forth.
...
<img src='../Images/ishago.png' style='float:right;width:40%'/>What some people today call *sacred geometry* our ancestors simply called geometry.  Contrary to what we learned in school, geometry is as much a study of philosophy as form, space, and mathematics.  Plato, Pythagoras, Parmenides, and several other great philosophers were well-schooled in geometry, but the marriage of philosophy and geometry goes back even further in history and across every culture.  Take the quadray coordinate system as an example.  This system is based on 60&deg;, which comes from the 360&deg; (60&deg; &times; 6) model of a circle.  One could make a case that the Ancient Mesopotamian base-6/60 number system, which gave us the 360° circle, was an early version of this idea.  In fact, one could make the case that the 20,000-44,000-year-old *Ishango Bone* of the upper paleolithic era discovered in the Congo was an early version of a base-60 system.  This bone is considered to be among the earliest instances of the concept of the number "1".  On this bone are counting marks, 60 marks on one side and 60 on the other.  One has to wonder if 60 was not chosen for a reason beyond what we might traditionally imagine.

*Synergetics* is a modern-day example of how geometry forms the basis of the structure of creation, life, and how everything interacts with each other.  Buckminster Fuller coined the term *synergetics* in his three-volume work "*Synergetics.  Explorations in the Geometry of Thinking*"[^70], wherein he explains how using a 60° coordinates system can explain both physics and chemistry, but more importantly, he believed that it also explained reality.  In his words:

> Since the physical Universe is entirely energetic, all dimensions must be energetic.  Synergetics is energetic geometry since it identifies energy with number.  Energetic geometry employs 60° coordination because that is nature's way to closest-pack spheres.
...
#### **Key 67:** Everything that exists *must* be perfect.
...
<img src='../Images/fig-q1.png' style='float: left;width:50px'/><img src='../Images/fig-q1a.png' style='float: left;width:50px'/><img src='../Images/spacer.png' style='float: left;height:40px'/>This is seen in a single complete tholon which consists of a *real* tholon (proton) and its *virtual* reflection tholon (neutron) as shown in ***Fig. q1*** and ***Fig. q1a*** below.  The upper trigram has a parent trigram at the top, which we can assign a value of -1/3, and the two children can be assigned a value of +2/3.  The center child trigram, which is always a reflection trigram, has a value of 0.  The opposite is true for the lower reflection trigram as well.  With these values, the upper tholon has a charge of 1, like a proton, and its reflection tholon has a charge of 0, like a neutron.  This also seems to indicate that a truly complete tholon requires two tholons: itself and its reflection tholon.  It is worth noting that the simplest element, hydrogen, is the only stable element that does not have neutrons (*heavy hydrogen*, or *deuterium*, has 1 neutron and is stable, and the radioactive *tritium* has 2 neutrons).  
...
<img src='../Images/fig-q1a.png' style='float:left;width:50px'/><img src='../Images/fig-q1.png' style='float:left;width:50px'/><img src='../Images/cristobalite_sm.png' style='float:left;height:44px'/><img src='../Images/spacer.png' style='float:left;height:40px'/>***Fig. q1a***, which is the folded 3D version of ***Fig. q1*** is also the same shape as the quartz *trans tridymite* silicon oxide molecule, with the N-states holding the same position as the silicon atom.  For those playing along, note that the *trans tridymite* structure contains 4 oxygen atoms (atomic number 8 each, totaling 32) and 2 silicon atoms (atomic number 14 each, totaling 28), resulting in a combined total of 60 protons.

<img src='../Images/fig-q3.png' style='float:left;width:50px'/><img src='../Images/spacer.png' style='float:left;height:40px'/>***Fig. q3*** is from the previously referenced E8 Lattice research[^501],[^502] that explains the various models within the E8 crystal and describes this shape as "representing all 8 components of the electron".  This is not exactly the same as what we are saying here, but in a different order, which supports the overarching idea regarding tetrahedra and subatomic particles.

[^501]: Smith, F.  D. (n.d.). **E8 Root Vectors from 8D to 3D** - Valdosta, Georgia. <http://www.valdostamuseum.com/hamsmith/E8to4Dand3D.pdf>
[^502]: Amazing description of how the E8 crystal can be defined as tetrahedrals. <https://quantumgravityresearch.org/portfolio/a-deep-link-between-3d-and-8d>
...
<img src='../Images/SNF2.png' style='float:right;width:50%' />Neutrons and protons are *hadrons*, and all *hadrons* are made of quarks.  A proton has 2 UP quarks and 1 DOWN quark, and a neutron has 1 UP quark and 2 DOWN quarks (*note: DOWN quarks are heavier than UP quarks, which contributes to why neutrons weigh more than protons*).  In the image on the right, you can see both the proton and the neutron as their component quarks.  Here we can see the same model applies, but in a different context.  An electron is not the product of a proton and a neutron; elements are their product.  It is more the product of a neutron, as neutrons decay into electrons, and in the model of the neutron, it would appear between the 2 DOWN quarks. 

Applying the 2D tholon maps in the context of the SNF, we see that within a single proton or neutron, the SNF holds the three quarks together at the center.  When protons and neutrons combine to form atomic nuclei, they interact through the residual strong force while maintaining their distinct identities.  In the tholonic model, those 6 N-state clusters in the center of the hexagons would appear as 6 separate N-states <img src='../Images/6dots.png' style='vertical-align: middle;width:30px' />, rather than 6 joined N-states <img src='../Images/6dots_joined.png' style='vertical-align: middle;height:30px' />, reflecting how nucleons remain distinct even while bound together. (What about the virtual tholons in this larger context? Do they instantiate as anti-matter?).
...
This pattern of 1, 2, 4 represents powers of 2 (2^0^, 2^1^, 2^2^), and would continue doubling forever for every new element added to the set.  For example, if there were four binary values instead of three, the representative number would be 8, then 16, then 32, and so on, each doubling the previous value.
...
We can see that the members of the groupings for protons (1,2,4) and neutrons (3,5,6) are identical to the values derived from the octahedron coordinates.  This is a very fractal or self-similar pattern because the 8 sides of the octahedron match the 8 possible configurations of *triquark* particles (any particle composed of 3 quarks, such as protons and neutrons, plus a bunch more).
...
Compare the current model of quarks as described using colors[^522] (Fig. v1, left) with the 2-dimensional tholon map of overlapping tholons showing 3 iterations (Fig. v2, right).  There are several similarities.  This is not to say the thologram is a new model of quantum physics, simply that we can find similar patterns.  The tholonic argument is that the quantum model is an instance of the thologram in the context of the quantum world.  Even the colors used to describe quarks are themselves a tholonic model in the context of frequencies of energies.  In this case, light, but the same applies to all frequencies of the electromagnetic spectrum.

<center><img src='../Images/quarks.png' style='width:100%'/></center>

[^522]: Greenberg, O.  W.  "**The Origin of Quark Color.**" Physics Today.  American Institute of PhysicsAIP, January 1, 2015.  https://physicstoday.scitation.org/doi/10.1063/PT.3.2655. 

**A point on stability and balance**

- It must be pointed out that *balanced* is not always the same as *stable*.  Stability depends on structure and balance, but depending on the structure of something, there is a certain limit of tolerance for imbalance.
...
We mentioned above that the first instance of matter would be tetrahedral, at least conceptually.  The previous example of the quark model as two tetrahedra demonstrates this: quarks form the hadrons from which all matter is composed, supporting the claim that tetrahedral structures underlie material reality.
...
The blue-green vector connects *gravity* to the *strong* force, representing *Definition and Limitation*.  Here we see an interesting contrast: while gravity and EM share the same mathematical formula pattern of $A=B\frac{Cc}{D^2}$ (for gravitational force it is $F=G\frac{Mm}{r^2}$, and for EM force it is $F=k\frac{Qq}{r^2}$), the gravity-to-strong-force vector spans from universal to atomic scales, defining the limits of scope and manifestation.  This is yet another example of self-similar patterns applied to different contexts.
...
In the tholonic model, when the 2D tholon map folds into 3D space, it creates the 1<sup>st</sup> level of the thologram, which is a plane of tetrahedra (both real tetrahedra projecting outward and virtual tetrahedra projecting inward).  This 1<sup>st</sup> level represents the fundamental pattern for the most basic building blocks of reality, regardless of scale.  

Due to the fractal nature of the thologram, this tetrahedral pattern exhibits bidirectional self-similarity.  Inward, the pattern self-replicates infinitely at ever-smaller scales (from atoms to subatomic particles, quarks, quantum fields, down to tetrahedral "pixels" at the Planck scale).  Outward, the pattern builds through successive iterations at ever-larger scales (1<sup>st</sup> Iteration, 2<sup>nd</sup> Iteration, and beyond, forming molecules, compounds, and material structures).  

At the material atomic scale, isolated elements are represented by individual tetrahedra within this framework, each exhibiting tholonic properties such as form seeking the most stable state.  Larger tetrahedral structures in matter emerge at the 1<sup>st</sup> Iteration and beyond, when elements bond to form molecular structures like water and methane.

A single *H<sub>2</sub>O* molecule, composed of 3 atoms, expresses itself as a trigram at the 2D level.  When water molecules interact and bond with other molecules, they form larger 3D tetrahedral structures.  This progression from 2D to 3D is analogous to how a 2D tholon map folds into a 3D tetrahedron when it creates children.  In both cases, the dimensional leap from 2D to 3D occurs through relationship and interaction.

<img src='../Images/haxagon.png' style='float:right;width:100%'/>

Water is a particularly good substance for exploring tholonic structures because it is one of the first, if not *the* first, tetrahedral structures that all life on Earth depends upon.  What makes water especially significant is its transformative nature: as an isolated H<sub>2</sub>O molecule, it exists as a 3-atom trigram (a 2D conceptual form), but when water molecules bond together, they form tetrahedral structures (a 3D form).  Water thus embodies both the 2D and 3D states, demonstrating the dimensional transformation central to the tholonic model.  In contrast, methane (CH<sub>4</sub>) is already tetrahedral as a single molecule, having 4 atoms arranged in tetrahedral geometry, already existing in its transformed 3D state.  Methane is one of the first organic molecules and not just a building block of life on Earth, but quite possibly the basis for an entirely different form of life (such as the azotosome cell membrane pictured in the image at the beginning of this section) that we might discover on the Saturn moon of Titan.

<img src='../Images/heliumcrystal.png' style='float: right;width:30%'/>It is worth noting an alternative cosmological perspective proposed by Nobel Prize-winning plasma physicist Hannes Alfvén,[^178] which suggests that the very original atom to come into existence was not hydrogen, but helium, with 2 protons, 2 neutrons, and 2 electrons.  As helium is the 1<sup>st</sup> *noble* gas, meaning it is perfectly balanced energetically, it would be less vulnerable to the wild energies present in the early universe.  Helium also forms a hexagonal crystal, so the helium crystal could represent the original form to come into existence in the Universe (image right).  Also significant in this model is that the 1<sup>st</sup> creation of matter, as atoms, would have started with 2 protons, not 1.

[^178]: Alfvén, H. (1986). "Cosmology: Myth or Science?" *IEEE Transactions on Plasma Science*, Vol. 14, No. 6.
...
<img src='../Images/085-h20.png' style='width:30px'/> H<sub>2</sub>O is composed of three atoms.  One atom is oxygen, which has an atomic number of 8 as it has 8 protons in its nucleus, which defines its nuclear charge.  Oxygen has 8 electrons total, with 6 in its outermost shell, but requires 8 electrons in its outer shell to achieve its most balanced state (the octet rule).  The other two atoms are hydrogen, the archetype of all atoms.  Hydrogen has an atomic number of 1 and has only 1 proton and 1 electron.  Because oxygen has only 6 electrons in its outer shell but wants 8, it naturally forms a bond with two hydrogen atoms by sharing the one electron each hydrogen has to offer.

<center><img src='../Images/tholon-atoms.png' style='float: right;width:50%'/></center>We can see a few similarities between the tholonic model of *H<sub>2</sub>O* and its traditional chemical model.  From a tholonic perspective, oxygen follows the pattern of 2 3<sup>rd</sup> gen tholons from 1 face of a 2<sup>nd</sup> gen tholon, which has 8 *real* tholons and 2 *virtual* tholons (***Fig. v1*** below, showing two 3<sup>rd</sup> gen tholons).  Hydrogen follows the pattern of a complete 2<sup>nd</sup> gen tholon (***Fig. v2***, showing 2 complete 2<sup>nd</sup> gen tholons).  Combining these 3 tholons (***Fig. v3***) would represent a total of 10 tholons, with 8 externally-facing points (from the *real* tholons) of interaction and 2 internally facing points (from the *virtual* tholons) of non-interaction.  When combined, the hydrogen tholons will attach to the empty or *virtual* tholons of oxygen.
...
The Fibonacci formula is especially tholonic.  For details, see "*Fibonacci Tholon*" in [Appendix C1, "*Tholonic Math*"](#Appendix-C1:-Tholonic-Math) *(Note: "Fibonacci Tholon" section to be added to Appendix C1)*.
...
- **Energy density** (E) is how much energy a battery can store in its mass (like a dam).  This is the *N-source* as the energy density represents the maximum potential of a battery's usability, and energy is the primary purpose of a battery.  This can also equate to *volts*, given that volts, N-sources, and energy density all represent the *potential* of energy.

- **Capital cost** (D) is the cost of the battery.  This is the *Defining* point because it is a point of *resistance*.  The cost will determine if it gets bought.

- **Operating cost** (C) is the cost to recharge the battery and the costs associated with keeping it safe and properly disposed of.  This is the *Contributing* point because this defines how the battery will integrate into the environment (of its use).  This can also equate to *current* and *acceleration*, as operating costs represent a consistent application of effort.

- **Power density** (P) is how much energy it can deliver relative to its mass (like the turbines in a dam).  This is the *white dot*, the *power*, or the amount of energy the battery can deliver over how much time.
...
According to the tholonic model, the only place where stable patterns capable of replication can occur is between the limitations of *Definition* and the integration of an instance (form) as *Contribution*, which here is **D** (capital cost) and **C** (operating cost).  Across this spectrum of "cooperation or conflict" is where price and value come to terms with each other, as in "getting the most for your money".  In the battery world (and many other worlds as well), this is the juggling act between good, cheap, and fast, and the arena that produces all the various instances of batteries that can deliver power at a cost, most of the instances landing in the "middle", representing the best product for the best price for a specific market.  Here would appear instances like alkaline cell batteries (Duracell), lead-acid batteries (car batteries), nickel-cadmium batteries (rechargeable batteries), etc.  Each of these new instances (stable patterns) can then go on to create their own tholonic archetypes.

### The Double Pendulum: A Mechanical Tholon

While we're speaking about instances, the double pendulum offers one of the most fascinating demonstrations of how the tholonic model mirrors reality. It exemplifies the N–D–C process in action, showing how chaos and order continuously transform into each other. The image below presents two perspectives of thousands of double pendulums positioned side by side, all released simultaneously with nearly identical initial conditions.[^535] Despite being as exact as possible in their setup, microscopic differences inevitably exist, and over time, these tiny variations amplify into divergent behaviors. What emerges are pockets of order that naturally arise within the chaotic system.

An intriguing thought experiment reveals the essential role of gravity in this system. Without gravity's constraint, the double pendulum would not exhibit the chaos-order dynamics we observe. Instead of swinging back and forth, the arms would simply rotate continuously, never returning to renegotiate their relationship. The chaotic behavior emerges not because gravity creates disorder, but because gravity creates the *bounded space* within which negotiation can occur. Remove the limitation, and you remove the arena where stable patterns can crystallize.

This demonstrates a fundamental tholonic principle: limitation is not the enemy of order; it is the precondition for order to emerge from chaos. In the double pendulum, gravity serves as the archetypal Definition that enables the Negotiation-Definition-Contribution cycle. Without this constraint, there is no recursion, no return, no opportunity for the system to explore the boundary between chaos and order. As we saw earlier in this chapter when examining the four fundamental forces, gravity may function as more than just a physical force and may be the first material instantiation of limitation itself, the primordial constraint that makes all subsequent creation possible.

<center><img src='../Images/dblpend.png' style='width:80%'/></center>

[^535]: Drew's Campfire. "**Pushing Simulation to the Limit to Find Order in Chaos**." YouTube, https://www.youtube.com/watch?v=8jVogdTJESw.

In tholonic terms, the first arm behaves as a parent tholon.  The second arm behaves as an n-child.  It inherits energy and constraints from the parent while adding a new freedom that the parent must now negotiate.  Gravity is the shared field enforcing limitation.  Motion is the contribution returned to the system.

Seen through this lens, the double pendulum is a minimal real-world instance of the tholonic model:

- **Awareness** → configuration (the system’s current angles and momentum)
- **Intention** → initial conditions (how it is released, from where, with what energy)
- **Negotiation** → energy exchange between arms under coupling
- **Limitation** → rod lengths, gravity, and conservation of energy
- **Contribution** → the trajectory traced through phase space

Nothing “random” is happening.  Everything is negotiated.

#### Sensitivity to initial conditions as negotiation amplification

What chaos theory calls *sensitivity to initial conditions* (a.k.a The Butterfly Effect) is, in tholonic language, recursive amplification of negotiated imbalance.  A microscopic asymmetry, a difference so small it is effectively invisible, feeds into a nonlinear negotiation loop and gets compounded at every cycle.  Each swing integrates the perturbation again.  Divergence is not a failure of determinism.  It is determinism operating in a negotiation space that cannot be linearly simplified.

This is why chaos is not disorder.  It is an over-negotiated order.

#### Islands of order as stable regimes (end-states)

The “islands of order” observed inside chaotic regions are stable regimes: configurations where the motion remains bounded and the system stays coherent.  Later, when we formalize end-states more precisely in the thologram, these correspond to what we call stable N-states.

What makes these regions special is not the absence of chaos; it is successful negotiation.  In those regions, contribution cycles remain bounded.  Energy does not run away into destabilizing transitions.  The system finds a configuration where:

Negotiation + Limitation + Contribution  
→ produces sustainability.

#### The flip as a boundary event

A flip is more than motion.  It is a topological transition.  In tholonic language, a flip represents a failure of negotiated constraint: energy exceeds the definition capacity of the current structure, and the system re-enters a renegotiation phase.

This explains why many chaotic trajectories involve flips and why many stable regions avoid them.  But the border is fractal and leaky.  Definition under recursion behaves as probabilistic rather than absolute.  Some systems survive flips and renegotiate.  Others never flip and still destabilize.  This is exactly how stable regimes behave near their sustainability limits.

#### Phase space as a slice of the Thologram

When we plot angle versus angle and find closed loops for ordered motion and scattered clouds for chaos, we are looking at a two-dimensional slice of a Thologram.  Ordered loops are self-similar recursive paths.  Chaotic scatter is unresolved negotiation space.  The transitions between them are phase boundaries inside a holarchy.

If we extend the phase space to include momentum, time, and energy flow, we approach a richer geometric structure, closer to the tholonic form than any single plot can reveal.

#### Determinism without predictability: awareness without foresight

The double pendulum is deterministic yet unpredictable.  This mirrors a core tholonic truth: awareness does not imply foresight.  Intention does not guarantee outcome.  Determinism does not imply simplicity.  Chaos emerges not from randomness, but from excess freedom under constraint.

The question the double pendulum keeps asking, where exactly is the border between order and chaos, is the same question every tholonic system asks at scale: where does recursion remain sustainable, and where does it collapse into renegotiation?

### Consciousness

The highly respected and published team of Donald David Hoffman (cognitive psychologist, author, and Professor in the Department of Cognitive Sciences at the University of California, Irvine) and Chetan Prakash (Ph.D.  Mathematical Physics, Cornell University) developed the idea of a *conscious agent*, which is a dynamic process of consciousness.  They argued that it is the interaction between various agents that produces measurable phenomena such as the position, momentum, and energy of objects.  They further argue that these objects have no objective, absolute, or even preexisting properties and only exist in relation to consciousness via these *conscious agents*.  This is a radical claim, but their arguments and evidence, as laid out in their paper "*Objects of consciousness*"[^77], cannot be ignored.  What also cannot be ignored is the striking similarity between their model and the tholonic model, as shown below.  (I found this especially striking as I learned of their work long after the 1<sup>st</sup> draft of this book.  Of course, they do a far better job of explaining their idea technically, mathematically, and psychologically.)
...
- The output of an RC circuit is a wave that has a unique form showing an exponential growth followed by an exponential drop, <img src='../Images/rcwave.png' style='height:15px'/>.  While the details of the form can change depending on the components, it always has the same characteristics of sharp ascending and descending.  The interesting thing about this waveform is that it can be perfectly recreated by quadrants of a circle.  Of course, just because two things have the same shape does not mean they share anything other than the shape, but in this case, it appears they share more.  At the least, if the RC circuit *is* an instance of a tholonic trigram, then we would expect its output to have some relationship to the thologram, and we do.

  *Note: The lowercase "n" represents a new child N-state that results from the children of the parent N-source.*

  <img src='../Images/tri-cir.png' style='float: right;width:30%'/>In the image to the right, we have divided this hypothetical circle into 4 quadrants and numbered them tholonically.  This means we use the *spatially* sequential values 0, 1, 3, 2 to represent the 4 points of a tholon.  (The *temporal* sequence is 0, 1, 2, 3 because 3 can only come into existence *after* 1 and 2, but once 3 exists, it does so spatially in between 1 and 2.)  By assigning these colors to those quadrants where 0 = blue (parent N-state), 1 = green (*Limitation*), 2 = red (*Contribution*), and 3 = yellow (child *n*-state), we find the same characteristics and patterns we see in the thologram, specifically, the 2D tholon map.  For example, the image below shows the output of an RC circuit as a result of the overlapping circles we just described.
  
  -  We see only the N-States (yellow and blue) overlap on their own color and alternate between parent → child → parent → child, etc., as we would expect.
  ...
  -  Every quadrant that is not exclusively an N-state contains all the other attributes (red, green, yellow), which alternate only along the path of the *lineage* (parent → child).
  -  Each circle in the row is alternately flipped on both the N*n* (blue-yellow) axis of parent-child N-states and across the DC (red-green) axis.
  -  The direction of the overlaps is the same as the path of the lineage in the tholon.
  -  The overlapping values are a product of 3 × 0, 1, 2, and in an order that creates a continuous oscillation <img src='../Images/036.png' style='height:25px'/>.

<center><img src='../Images/rcgrid.png' style='width:100%'/></center>

- In the tholon, the field in which the *n*-state can be created exists between D and C.  Here, that field is defined by *R* and *C*, which creates a field of *time*.  In electronics, it is called the *time constant* and is written as ***τ = RC*** (where ***τ*** is the Greek *tau* representing the *time constant*).  This time constant determines how voltage (*V*) and current (*I*) change exponentially over time, with voltage increasing as current decreases during the charging process.

<center><img src='../Images/rcgraphs.png' style='width:80%'/></center>

This *time constant* is then used to calculate *V* and *I* using the formulas *V(τ)=V<sub>B</sub>(1-e<sup>-τ/RC</sup>)* and *I(τ)=I<sub>0</sub>(e<sup>-τ/RC</sup>)*.  The point here is not to get into the math but to show not only that *V* and *I* relate to each other via *R* and *C*, and by extension, *D* and *C*, but that they do so across a field that in the world of electronics is defined by ***e***, or 2.71828, the base of the *natural logarithm*.  This is especially interesting when applying the properties of this RC field to that of the DC field of the tholon because ***e*** is one of the most profound and fascinating numbers in nature!
...
If you remember back in chapter 3, it was shown how logarithms were the natural way nature understands scale or scope, and while *common logs* are based on 10, the *natural log* uses ***e*** as its base.  It is called "natural" because ***e*** emerges naturally from continuous growth processes, making it the unique number where the rate of growth equals the value itself (the derivative of ***e<sup>x</sup>*** is ***e<sup>x</sup>***).
...
If ***e<sup>iπ</sup> = -1***, then ***ln(-1) = πi***.  When we apply the concepts of ***e*** and ***ln()*** as described above, ***πi*** equates to time, and the spectrum of ***e*** represents the growth.  Does this also suggest that the originating energy of everything, including the Universe, started with a negative movement (the -1 of *e<sup>iπ</sup>*)?  There is more evidence to suggest that this is the case, and later it is shown why and what that really means.
...
What this chapter has demonstrated is that instantiation is not a mysterious creative act but a structural consequence of the N-D-C pattern operating in context.  From quark charge distributions to molecular geometry, from double pendulums to RC circuits and Euler's identity, the same tholonic architecture reappears at every scale and in every domain.  Every instance is as perfect an expression of its archetype as its context allows, and every archetype is waiting for the conditions that will allow it to manifest.

Chapter 14 takes this structural understanding and applies it systematically across physics, biology, society, economics, and psychology, demonstrating that the N-D-C pattern is not simply a feature of examples selected to illustrate it, but a universal constraint on any system that sustains itself in existence.
